...
| Tag (per word match) | +2 | +1 | Always split, deduplicated per entry |
| Summary (full phrase substring) | +1 | +1 | Unchanged |
| Content exact phrase (rg) | +3 | +word count | New — high-value match |
| Content individual word fallback (rg) | +1 | +1 per word | New — per word that hits a file |
...
tags:      split term into words, match each word against each tag
```
...
1. Index phase: split tags for multi-word terms
   ...
   - Split the term on whitespace, check each word against each tag
   ...
   Tasks:
   - Change tag matching to split multi-word terms into words before comparing
   - Add test: multi-word term with individual words matching tags scores 2
   - Add test: single-word term tag matching unchanged
   Deliverable: Multi-word terms match against individual tags
...
2. Content phase: exact phrase then word fallback
   ...
   - If no hits: split into words, call `_rg_search` per word, score +1 per word-hit, matches +1 per word-hit
   ...
   - Implement word-split fallback with +1 per word
