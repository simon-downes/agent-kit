...
from agent_kit.brain.index import (
    _extract_metadata,
    _parse_frontmatter,
    _slug_to_name,
)
from agent_kit.brain.search import _match_weight
...
class TestConfiguredContexts:
    def test_returns_contexts(self):
        ...
        assert BrainClient.configured_contexts(cfg) == {"work": "git@host:repo"}

    def test_empty(self):
        assert BrainClient.configured_contexts({}) == {}
...
class TestListContexts:
    def test_lists_dirs(self, tmp_path):
        ...
        (tmp_path / ".git").mkdir()
        assert BrainClient(tmp_path).list_contexts() == ["shared", "work"]

    def test_missing_dir(self, tmp_path):
        assert BrainClient(tmp_path / "nope").list_contexts() == []
...
class TestLoadIndex:
    def test_loads_yaml(self, tmp_path):
        ...
        assert BrainClient(tmp_path).load_index(tmp_path) == idx

    def test_missing_returns_empty(self, tmp_path):
        assert BrainClient(tmp_path).load_index(tmp_path) == {}

    def test_invalid_yaml_raises(self, tmp_path):
        (tmp_path / "index.yaml").write_text(":\n  :\n  - :\n  bad: [")
        with pytest.raises(ValueError, match="invalid index.yaml"):
            BrainClient(tmp_path).load_index(tmp_path)
...
class TestQueryIndex:
    ...
    def test_by_type(self):
        assert self.client.query_index(self.index, entity_type="contacts") == {
            "contacts": {"alice": {"name": "Alice"}}
        }
...
class TestInitContext:
    ...
    def test_creates_local(self, mock_run, tmp_path):
        ...
        result = BrainClient(tmp_path).init_context("shared")
    ...
    def test_clones_repo(self, mock_run, tmp_path):
        mock_run.return_value = MagicMock(returncode=0, stderr="")
        result = BrainClient(tmp_path).init_context("work", "git@host:repo")
    ...
    def test_clone_failure(self, mock_run, tmp_path):
        ...
        with pytest.raises(ValueError, match="git clone failed"):
            BrainClient(tmp_path).init_context("work", "git@host:bad")

    def test_already_exists(self, tmp_path):
        (tmp_path / "shared").mkdir()
        assert BrainClient(tmp_path).init_context("shared") is None

    def test_invalid_name(self, tmp_path):
        with pytest.raises(ValueError, match="invalid context name"):
            BrainClient(tmp_path).init_context(".bad")
    ...
    def test_git_init_failure(self, mock_run, tmp_path):
        ...
        with pytest.raises(ValueError, match="git init failed"):
            BrainClient(tmp_path).init_context("shared")
...
class TestInitBrain:
    ...
    def test_creates_structure(self, mock_run, tmp_path):
        mock_run.return_value = MagicMock(returncode=0, stderr="")
        actions = BrainClient(tmp_path).init_brain({"brain": {"contexts": {}}})
...
class TestContextStatus:
    ...
    def test_clean(self, mock_run, tmp_path):
        ...
        result = BrainClient(tmp_path).context_status(tmp_path)
...
class TestValidateContext:
    def test_missing_dirs(self, tmp_path):
        ctx = tmp_path / "test"
        ctx.mkdir()
        findings = BrainClient(tmp_path).validate_context(ctx)
...
class TestValidateOrigins:
    ...
    def test_matching_origin(self, mock_run, tmp_path):
        ...
        cfg = {"brain": {"contexts": {"work": "git@host:repo"}}}
        findings = BrainClient(tmp_path).validate_origins(cfg)
        assert findings == []
    ...
    def test_skips_no_repo(self, tmp_path):
        ...
        assert BrainClient(tmp_path).validate_origins(cfg) == []
...
class TestReindexContext:
    ...
    def test_indexes_markdown_file(self, mock_flock, tmp_path):
        ...
        result = BrainClient(tmp_path).reindex_context(ctx)
...
class TestCommitContext:
    ...
    def test_commits_all(self, mock_run, tmp_path):
        ...
        sha = BrainClient(tmp_path).commit_context(tmp_path, "update")
        assert sha == "abc1234"

    @patch("agent_kit.brain.git.subprocess.run")
    def test_nothing_to_commit(self, mock_run, tmp_path):
        mock_run.return_value = MagicMock(returncode=0, stdout="")
        assert BrainClient(tmp_path).commit_context(tmp_path, "update") is None
    ...
    def test_specific_paths(self, mock_run, tmp_path):
        ...
        sha = BrainClient(tmp_path).commit_context(tmp_path, "update", ["file.md"])
        assert sha == "def5678"
    ...
    def test_add_failure(self, mock_run, tmp_path):
        ...
        with pytest.raises(ValueError, match="git add failed"):
            BrainClient(tmp_path).commit_context(tmp_path, "update")
...
class TestSearchBrain:
    ...
    def test_specific_context(self, tmp_path):
        ...
        with patch("agent_kit.brain.search._rg_search", return_value=[]):
            results = BrainClient(tmp_path).search("alice", context="work")
...
class TestContextLock:
    ...
    def test_lock_acquired_and_released(self, mock_flock, tmp_path):
        ...
        for d in ENTITY_DIRS:
            (ctx / d).mkdir(parents=True)
        BrainClient(tmp_path).reindex_context(ctx)
